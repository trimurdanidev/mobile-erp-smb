    ...
    <ion-header class="app-header" :translucent="false">
      <div class="header-card">
        ...
        <div class="header-text">
          <span class="header-title">Laporan By Admin</span>
          <span class="header-sub">Laporan resi berdasarkan admin / user</span>
        </div>
      </div>
    </ion-header>
          ...
          <div class="filter-group">
            ...
            <div class="select-wrapper">
              <select v-model="selectedAdmin" class="filter-select">
                ...
                <option v-for="a in adminList" :key="a.id" :value="a.id">
                  {{ a.name }}
                </option>
              </select>
              <ion-icon
        ...
        <div v-if="hasResult" class="result-card">
          ...
          <div class="table-wrap">
            <div class="table-head">
              ...
              <span class="col-admin">Karyawan</span>
            ...
            <template v-for="(group, gi) in groupedData" :key="gi">
              <div class="table-row row-group">
                ...
                <span class="col-admin admin-name">{{ group.admin }}</span>
...
export default {
  ...
  setup() {
    ...
    const totalResi = computed(() =>
      laporanData.value.reduce((sum, g) => sum + (g.total || 0), 0)
    );
    ...
    const loadAdminList = async () => {
      try {
        const res = await api.get("/resi/getListAdmin");
        adminList.value = res.data.data || [];
      } catch (e) {
        showToast("Gagal memuat daftar admin", "danger");
      }
    };
    ...
    const loadLaporan = async () => {
      ...
      try {
        const params = {
          ...
          user_id: selectedAdmin.value || undefined,
          ekspedisi: selectedEkspedisi.value || undefined,
        };
        const res = await api.get("/resi/getLaporanByAdmin", { params });
