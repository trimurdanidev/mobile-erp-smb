      ...
      <div class="scan-wrapper">
        ...
        <div class="scan-card">
          ...
          <div class="camera-box" :class="{ 'camera-active': isScanning }">
            ...
            <div class="camera-placeholder" v-if="!isScanning">
              ...
              <div class="barcode-circle">
                <ion-icon :icon="barcodeOutline" class="placeholder-icon"></ion-icon>
              </div>
              <p class="cam-placeholder-text">Siap untuk scan barcode</p>
              <span class="cam-hint-badge">Tap "Mulai Scan Barcode" untuk mulai</span>
            </div>
          </div>
          ...
          <ion-button
            ...
            @click="toggleScan"
          >
            <ion-icon :icon="isScanning ? stopOutline : scanOutline" slot="start"></ion-icon>
        ...
        <div class="history-card" v-if="scanHistory.length > 0">
          <div class="scan-card-header" style="border-bottom: 1px solid #f1f5f9">
            <ion-icon
              :icon="listOutline"
              class="scan-section-icon"
              style="color: #ea580c; background: #fff7ed"
            ></ion-icon>
            <span class="scan-section-title">Riwayat Scan ({{ scanHistory.length }})</span>
          </div>
          <div class="history-list">
            ...
            >
              <ion-icon
                :icon="item.success ? checkmarkCircleOutline : closeCircleOutline"
                class="history-icon"
              ></ion-icon>
        ...
        <div class="result-card" v-if="lastResult">
          ...
          >
            <ion-icon
              :icon="lastResult.success ? checkmarkCircleOutline : closeCircleOutline"
            ></ion-icon>
          </div>
          <div class="result-text">
            <div class="result-title">{{ lastResult.success ? "Berhasil!" : "Gagal" }}</div>
            <div class="result-msg">{{ lastResult.message }}</div>
            <div class="result-resi" v-if="lastResult.no">No: {{ lastResult.no }}</div>
          </div>
        </div>
...
const openCamera = async (): Promise<boolean> => {
  if (!navigator.mediaDevices?.getUserMedia) {
    await showToast("Kamera tidak tersedia. Pastikan akses via HTTPS.", "danger");
    return false;
  }
  ...
  if (!stream) {
    await showToast("Gagal akses kamera. Pastikan izin kamera sudah diberikan.", "danger");
    return false;
  }
...
const startZXingScanner = async () => {
  ...
  try {
    ...
    if (videoRef.value && stream) {
      await zxingReader.decodeFromStream(stream, videoRef.value, async (result: any, err: any) => {
    ...
    await showToast("Scanner tidak dapat dimuat. Coba gunakan Input Manual.", "warning");
  }
};
...
.scan-app-header::after { display: none !important; }
.scan-toolbar {
...
.toolbar-title { font-size: 17px; font-weight: 800; color: #fff; line-height: 1.2; }
.toolbar-subtitle { font-size: 11px; color: rgba(255,255,255,0.6); font-weight: 500; }

ion-content { --background: #f0f4f8; }
.scan-wrapper { padding: 16px; display: flex; flex-direction: column; gap: 14px; }

.scan-card { background: #fff; border-radius: 20px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.07); }
.scan-card-header { display: flex; align-items: center; gap: 8px; padding: 14px 16px 12px; }
.scan-section-icon { font-size: 18px; padding: 6px; border-radius: 8px; }
.scan-section-title { font-size: 13px; font-weight: 700; color: #334155; }
...
.barcode-circle {
  ...
  box-shadow: 0 0 0 8px rgba(37,99,235,0.15), 0 8px 24px rgba(37,99,235,0.4);
  z-index: 1;
  margin-bottom: 14px;
}
.placeholder-icon { font-size: 34px; color: #fff; filter: drop-shadow(0 2px 6px rgba(0,0,0,0.3)); }
.cam-placeholder-text { font-size: 13px; font-weight: 600; color: rgba(255,255,255,0.75); margin: 0 0 10px; z-index: 1; }
.cam-hint-badge {
...
.scan-line {
  ...
  background: #f97316; top: 50%;
  animation: scanMove 2s ease-in-out infinite;
  box-shadow: 0 0 8px #f97316;
}
@keyframes scanMove { 0%,100% { top: 25%; } 50% { top: 75%; } }
.scan-corner { position: absolute; width: 20px; height: 20px; border-color: #f97316; border-style: solid; }
.scan-corner.tl { top: 16px; left: 16px; border-width: 3px 0 0 3px; }
.scan-corner.tr { top: 16px; right: 16px; border-width: 3px 3px 0 0; }
.scan-corner.bl { bottom: 16px; left: 16px; border-width: 0 0 3px 3px; }
.scan-corner.br { bottom: 16px; right: 16px; border-width: 0 3px 3px 0; }

.scan-btn { margin: 12px 16px; --border-radius: 14px; font-weight: 700; }

.input-row { padding: 14px 16px; }
.resi-input {
...
.resi-input:focus { border-color: #ea580c; }

.history-card { background: #fff; border-radius: 20px; overflow: hidden; box-shadow: 0 2px 12px rgba(0,0,0,0.07); }
.history-list { max-height: 200px; overflow-y: auto; }
.history-item { display: flex; align-items: center; gap: 10px; padding: 10px 16px; border-bottom: 1px solid #f8fafc; }
.history-item:last-child { border-bottom: none; }
.history-ok { background: #f0fdf4; }
.history-err { background: #fef2f2; }
.history-icon { font-size: 18px; flex-shrink: 0; }
.history-ok .history-icon { color: #16a34a; }
.history-err .history-icon { color: #dc2626; }
.history-text { flex: 1; min-width: 0; }
.history-no { font-size: 12px; font-weight: 700; color: #1e293b; font-family: monospace; }
.history-msg { font-size: 11px; color: #64748b; }
.history-time { font-size: 10px; color: #94a3b8; white-space: nowrap; }

.result-card { background: #fff; border-radius: 16px; padding: 16px; display: flex; align-items: center; gap: 14px; box-shadow: 0 2px 12px rgba(0,0,0,0.07); }
.result-icon-wrap { width: 44px; height: 44px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 24px; flex-shrink: 0; }
...
.result-msg { font-size: 12px; color: #64748b; margin-top: 2px; }
.result-resi { font-size: 11px; color: #94a3b8; margin-top: 4px; font-family: monospace; }
...
.submit-btn:active:not(.submit-loading) { transform: scale(0.97); }
.submit-btn:disabled { opacity: 0.6; cursor: not-allowed; }
.submit-loading { opacity: 0.75; }
.btn-icon { font-size: 18px; }
.btn-spinner { --color: #fff; width: 18px; height: 18px; }
</style>